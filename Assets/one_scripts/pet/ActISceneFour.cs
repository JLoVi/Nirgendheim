...
public class ActISceneFour : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (rotate == true && Input.GetKey(GamepadController.instance.controlList.closeReopenControl))
        {
        ...
        if (ofcourse == true && Input.GetKey(GamepadController.instance.controlList.closeReopenControl))
        {
        ...
        if (camback == true && Input.GetKey(GamepadController.instance.controlList.closeReopenControl)) ///touchpadra attenni
        {
            camLookBack(10F);
    ...
    private IEnumerator MyEvent()
    {
        ...
        //  xImg.SetActive(true);
        string s = string.Format("PRESS <color=#00ff00ff>{0}</color> IF YOU'D LIKE TO CHANGE SOME OF THE PARAMETERS", GamepadController.instance.controlList.closeReopenName);
        infoText.text = s;
        rotate = true;
        ofcourse = true;
       ...
       // Application.LoadLevel("ActIScene5");
        SceneManager.LoadScene("ActIScene5");
